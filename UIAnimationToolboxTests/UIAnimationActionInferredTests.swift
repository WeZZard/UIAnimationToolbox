...
class UIAnimationActionInferredTests: XCTestCase {
    // MARK: - Init with Layer Event
    func testInitWithLayerEvent_returnsNil_whenCalledOutsideAnimationBlock() {
        let layer = CALayer()
        let action = UIAnimationActionInferred(layer: layer, event: "bounds.size")
        XCTAssertNil(action)
    }
    ...
    func testInitWithLayerEvent_returnsNonNil_whenCalledInsideBlockOfBlockAnimation5() {
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: [], animations: {
            action = UIAnimationActionInferred(layer: layer, event: "bounds.size")
        }, completion: nil)
    ...
    func testMakeLayerEvent_returnsInstanceOfNSNull_whenCalledOutsideAnimationBlock() {
        let layer = CALayer()
        let action = UIAnimationActionInferred.make(layer: layer, event: "bounds.size")
    ...
    func testMakeLayerEvent_returnsInstanceOfCAAction_whenCalledInsideBlockOfBlockAnimation5() {
        ...
        UIView.animate(withDuration: 0.3, delay: 0, options: [], animations: {
            action = UIAnimationActionInferred.make(layer: layer, event: "bounds.size")
        })
